...
Blowfish::Blowfish(vector<uint8_t> key){
    ...
    _pArray = {0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0,
    ...
    //Modify _pArray and _sBox according to key
    setKey(key);
...
void Blowfish::setKey(vector<uint8_t> key){
    //Tells us how many bits the variable length key is
    int nKey = key.size() * 8;

    //XOR key data into pArray

    //Create 18 32 bit entries by looping the key 
...
void Blowfish::encrypt(uint64_t & data) const{
    ...
    LHS = inRHS ^ _pArray[17];

    RHS = inLHS ^ _pArray[16];

    //Convert LHS into 64 bit and shift left by 32. Perform bitwise OR to add RHS
